...
export function Knowledge({ limit = 4 }: { limit?: number }) {
    ...
    return (
        <section id="recursos" className="py-16 md:py-24 border-t border-line bg-surface">
                    ...
                    {articles.map((a, i) => (
                        <RevealWrapper key={a.slug} delay={i * 0.06}>
                            <Link href={`/recursos/${a.slug}`} className="card group flex h-full flex-col p-5">
                                ...
                                <h3 className="mt-2 flex-1 text-base font-semibold leading-snug text-ink group-hover:text-accent transition-colors">{a.title}</h3>
